...
public class PaymentService {
    ...
    public void successPayment(PaymentRequest request) throws AppException {
        ...
        if (request.getType() == TypeTransactionEnum.ORDER) {
            ...
            appointment.setStatus(com.dapp.backend.enums.AppointmentStatus.SCHEDULED);
    ...
    public void cancelPayment(PaymentRequest request) throws AppException {
        ...
        if (request.getType() == TypeTransactionEnum.ORDER) {
            ...
            Appointment appointment = appointmentRepository.findById(Long.parseLong(request.getReferenceId()))
                    .orElseThrow(() -> new AppException("Appointment not found!"));
            appointment.setStatus(com.dapp.backend.enums.AppointmentStatus.CANCELLED);
            appointmentRepository.save(appointment);
            payment.setReferenceType(request.getType());
        }
    ...
    public com.dapp.backend.dto.response.TransactionResultResponse getTransactionResult(Long paymentId)
            throws AppException {
        Payment payment = paymentRepository.findById(paymentId)
                .orElseThrow(() -> new AppException("Payment not found!"));

        com.dapp.backend.dto.response.TransactionResultResponse.TransactionResultResponseBuilder builder = com.dapp.backend.dto.response.TransactionResultResponse
                .builder()
        ...
        if (payment.getReferenceType() == TypeTransactionEnum.APPOINTMENT) {
            ...
            builder.vaccineName(appointment.getVaccine() != null ? appointment.getVaccine().getName() : "N/A")
                    .centerName(appointment.getCenter() != null ? appointment.getCenter().getName() : "N/A")
                    .scheduledDate(appointment.getScheduledDate())
                    .scheduledTime(
                            appointment.getScheduledTimeSlot() != null ? appointment.getScheduledTimeSlot().toString()
                                    : "")
