...
import { Table, Card, Tag, Button, Space, message, Tooltip, Modal, Form, Select, Input, DatePicker } from 'antd';
import {
    EditOutlined,
    CheckCircleOutlined,
    ClockCircleOutlined,
...
import { callAllAppointment, callCancelAppointment, callCompleteAppointment, callConfirmAppointment, callRefundAppointment } from '../../config/api.appointment';
...
const AppointmentManagementPage = () => {
    ...
    const {
        ...
        async () => {
            const response = await callAllAppointment();
    ...
    const getStatusTag = (status) => {
        switch (status) {
            ...
            case 1:
                return <Tag icon={<CheckCircleOutlined />} color="processing">Đã xác nhận</Tag>;
            case 2:
                return <Tag icon={<CheckCircleOutlined />} color="success">Đã tiêm</Tag>;
            case 3:
                return <Tag icon={<CloseCircleOutlined />} color="error">Đã hủy</Tag>;
            case 4:
                return <Tag icon={<CloseCircleOutlined />} color="default">Đã hoàn tiền</Tag>;
            default:
                return <Tag icon={<ExclamationCircleOutlined />} color="default">Không xác định</Tag>;
        }
    };
        ...
        confirm({
            ...
            content: (
                <div>
                    ...
                    <p>Số tiền: {new Intl.NumberFormat('vi-VN', {
                        style: 'currency',
                        currency: 'VND'
                    }).format(price)}</p>
                    <p className="text-blue-600 font-semibold">Tương đương: {ethAmount} ETH</p>
                </div>
            ),
            async onOk() {
                    ...
                    <Space>
                        ...
                        {record.status === 3 && (
                            <Button
                                ...
                                danger
                                size="small"
                                onClick={() => handleRefund(record.patientAddress, record.appointmentId, record.price)}
                            >Hoàn tiền</Button>
                        )}
                    </Space>
                );
